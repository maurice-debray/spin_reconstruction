...
a_par_weights = np.array(get_config("reconstruction_full", ["cost", "a_par_refs"])) ** (
    -2
)
tolerance = get_config("reconstruction_full", ["cost", "tolerance"])
nb_par_weight = gamma_ratio**2
nb_tolerance = tolerance / gamma_ratio
cutoff = get_config("reconstruction_full", ["cost", "cutoff"])

file = get_config("reconstruction_full", ["filename"])
couplings_file = get_config("reconstruction_full", ["couplings_file"])
all_B = get_config_default("reconstruction_full", ["all_B"], True)
coupling_keys = get_config_default("reconstruction_full", ["coupling_keys"], [])
...
def compute_sites(
    ...
    couplings, permutation = set_placing_order(
        couplings.copy(), first=int(np.argmax(np.abs(nb_par_data)))
    )
    a_par_data = a_par_data[permutation]
    nb_par_data = nb_par_data[permutation]
    possible_configurations = np.array(
        [[i] + [0] * n_tot for i in range(max_distance**3 * site_nb)],
        dtype=np.uint64,
    )
    ...
    while n_placed < n_tot + 1:
        if verbose:
            print(
                f"Placing {n_placed}. {len(possible_configurations)}*{len(all_couplings)} cases to process."
            )
            print(f"Current config {possible_configurations[0]}")
        new_possible_configurations = compute_new_possible_config(
            possible_configurations, len(all_couplings) + 1, n_tot + 1, n_placed
        )
        checkpoint = (
    ...
    with h5py.File(file, "w") as g:
        ...
        for a_par_weight in tqdm(a_par_weights):
            ...
            for key, v in f.items():
                ...
                nb_par_full = v["NB_couplings_full"][:]
